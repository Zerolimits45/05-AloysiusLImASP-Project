...
class Asia5:
    imva = pd.read_excel('IMVA.xls', thousands=',')  # import xls file that is in the same folder
    ...
    for col_id in columns_in_asia_5_as_INDEX:
        ...
        for value_index in range(len(asia5[col_id])):
            if asia5[col_id][value_index] == asia5[col_id][396] or value_index > 396:  # value_index is the integer index for whole 516 #forgot to set it up
                ...
                sum_of_each_col.append(int(asia5[col_id][value_index])) # use this for months and years too

        all_SUM_of_asia5.append(sum(sum_of_each_col))
        all_COUNTRY_of_asia5.append(asia5[col_id].name)

    global country  # to enable the iterable loop on line 78 to be available
    global n  # to enable the iterable loop on line 77 to be available
    for country in asia5:
        months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October',
                  'November', 'December']
        df_each_year = pd.DataFrame(columns=months)
        print(country.name)
        n = 12  # set number of months
        each_year = [country[i * n:(i + 1) * n] for i in range(
            (len(country) + n - 1) // n)]  # becomes a list of LISTS with all exhaustive years 1978 - 2020 : 43 years
        count = 0
        for year in each_year:
            # print(list(year) )
            df_length = len(df_each_year)
            df_each_year.loc[df_length] = list(year)
            count = count + 1
            # print(count)
        df_each_year.index = df_each_year.index + 1978
        df_each_year = df_each_year.replace('na', 0)
        sum_of_each_year = df_each_year.sum(axis=1)
        # print(sum_of_each_year)
        print(df_each_year)
